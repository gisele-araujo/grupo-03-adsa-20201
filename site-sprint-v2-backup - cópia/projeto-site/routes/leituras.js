...
router.get('/dadosMemoria', (request, response) => {
    var sql = `select top(5) memoriaUsada, FORMAT(dataHora,'dd/MM/yyyy HH:mm') as dataHora from DadosGerados order by idDados desc;`;
...
router.get('/dadosDisco', (request, response) => {
    var sql = `select top(5) discoUsado, FORMAT(dataHora,'dd/MM/yyyy HH:mm') as dataHora from DadosGerados order by idDados desc;`;
    db.query(sql, function (err, result) {
        if (err) throw err;
        response.json(result);
    });
